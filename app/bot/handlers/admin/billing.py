...
async def _render_billing_text(
    session: AsyncSession, tenant_id: uuid.UUID, staff_lang: str
) -> str:
    subscription = await SubscriptionService(session, tenant_id).get_or_create_default()
    plan = subscription.plan
    status = status_label(subscription.status, staff_lang)
    lines = [
        t("billing.screen_title", staff_lang),
        "",
        t("billing.current_plan", staff_lang, plan_name=esc(plan.name)),
        t("billing.subscription_status", staff_lang, status=status),
        "",
        t("billing.features_header", staff_lang),
    ]
    ...
    for feature in Feature:
        ...
        lines.append(t(key, staff_lang, feature_name=feature_label(feature, staff_lang)))
    lines.append("")
    lines.append(t("billing.limits_header", staff_lang))
    ...
    for limit_key in LimitKey:
        ...
        name = limit_label(limit_key, staff_lang)
        if maximum is None:
            lines.append(
                t("billing.limit_line_unlimited", staff_lang, limit_name=name, current=current)
            )
        else:
            lines.append(
                t(
...
async def _billing_kb(
    session: AsyncSession, *, is_super_admin: bool, staff_lang: str
) -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
    if is_super_admin:
        for plan in await PlanRepository(session).list_all():
            builder.button(
                text=t("billing.dev_change_plan_btn", staff_lang, plan_name=esc(plan.name)),
                callback_data=AdmCB(action="billing_set", arg=plan.code),
            )
        builder.adjust(1)
    builder.row(
        InlineKeyboardButton(
            text=t("admin.btn.back", staff_lang), callback_data=AdmCB(action="menu").pack()
        )
    )
...
async def show_billing(
    ...
    text = await _render_billing_text(session, tenant_id, staff_lang)
    markup = await _billing_kb(session, is_super_admin=is_super_admin, staff_lang=staff_lang)
...
async def dev_change_plan(
    ...
    try:
        ...
        await alert(callback, t("billing.try_again", staff_lang))
        return
    await alert(
        callback,
        t("billing.plan_changed", staff_lang, plan_name=esc(subscription.plan.name)),
    )
    text = await _render_billing_text(session, tenant_id, staff_lang)
    markup = await _billing_kb(session, is_super_admin=is_super_admin, staff_lang=staff_lang)
    await edit_message(callback, text, markup)
